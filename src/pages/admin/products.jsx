...
export function AdminProductsPage() {
  ...
  const [managedProducts, setManagedProducts] = useState(
    CATALOG.map((p) => ({
      ...p,
      status: p.badge === "Sale" ? "Active" : p.badge === "New" ? "Active" : "Active",
                ...
                return (
                  <tr key={product.id} className={cn("hover:bg-slate-50/70 transition-colors", selectedIds.includes(product.id) && "bg-violet-50/30")}>
                    ...
                    <td className="p-4 hidden md:table-cell">
                      <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-semibold text-slate-600">{product.category}</span>
                    </td>
