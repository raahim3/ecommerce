...
export function AdminReportsPage() {
  ...
  const data = REPORT_DATA[dateRange] || REPORT_DATA["Last 7 Days"];
  const prevData = REPORT_DATA["Last 7 Days"];

  const handleExportPDF = () => toast.success("Generating PDF Executive Summary Report...");
  const handleExportCSV = () => toast.success("Exporting raw analytics CSV data...");

  const maxBarRevenue = Math.max(...(data.dailyRevenue || []));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Complete financial performance dashboard with export capabilities.</p>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            <div key={item.label} className={cn("rounded-2xl p-4 text-center border", item.bg, item.bold ? "border-emerald-300 ring-1 ring-emerald-200" : "border-slate-200/60")}>
              ...
              <p className={cn("text-lg font-extrabold mt-1", item.color)}>
                {item.val < 0 ? "-" : ""}{formatPrice(Math.abs(item.val))}
              </p>
            </div>
          ))}
        </div>
      ...
      {/* Revenue Bar Chart + Key Metrics */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Revenue Bar Chart (8 cols) */}
        <div className="lg:col-span-8 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs">
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
            <h2 className="text-base font-bold text-slate-900">Daily Revenue Breakdown</h2>
            <span className="text-xs font-bold text-emerald-600 flex items-center gap-1">
              <TrendingUp className="size-3.5" /> +18.4% vs prior period
            </span>
          </div>

          {/* SVG Bar Chart */}
          <div className="h-52 flex items-end gap-1.5 overflow-hidden">
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-1 min-w-0 group relative cursor-pointer" onMouseEnter={() => setHoveredBar(i)} onMouseLeave={() => setHoveredBar(null)}>
                  {hoveredBar === i && (
                    <div className="absolute -top-10 left-1/2 -translate-x-1/2 rounded-xl bg-slate-900 text-white text-[10px] font-bold px-2 py-1 whitespace-nowrap shadow-lg z-10">
                      {data.dailyLabels[i]}: {formatPrice(val)}
                    </div>
                  )}
                  <div
                  ...
                  {data.dailyRevenue.length <= 10 && (
                    <span className="text-[9px] font-semibold text-slate-400 truncate">{data.dailyLabels[i]}</span>
                  )}
                </div>
              );
            })}
        ...
        <div className="lg:col-span-4 space-y-4">
          {[
            { label: "Orders Placed", val: data.orderCount.toLocaleString(), icon: ShoppingBag, color: "text-violet-700 bg-violet-100" },
            { label: "Unique Customers", val: data.customerCount.toLocaleString(), icon: Users, color: "text-sky-700 bg-sky-100" },
            { label: "New Customers", val: data.newCustomers.toLocaleString(), icon: TrendingUp, color: "text-emerald-700 bg-emerald-100" },
            { label: "Average Order Value", val: formatPrice(data.avgOrder), icon: DollarSign, color: "text-amber-700 bg-amber-100" },
            { label: "Conversion Rate", val: `${data.conversionRate}%`, icon: BarChart3, color: "text-slate-700 bg-slate-100" },
          ].map((metric) => (
