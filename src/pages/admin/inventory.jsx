  ...
  const handleApplyAdjust = () => {
    ...
    setInventory((prev) =>
      prev.map((item) => {
        ...
        let newQty = item.onHand;
        if (adjustType === "set") newQty = Number(adjustQty);
        else if (adjustType === "add") newQty = item.onHand + Number(adjustQty);
        else if (adjustType === "subtract") newQty = Math.max(0, item.onHand - Number(adjustQty));
