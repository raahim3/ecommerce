...
export function AdminDashboardPage() {
  ...
  const activeChartData = timeRange === "monthly" ? REVENUE_DATA_MONTHLY : REVENUE_DATA_WEEKLY;
  ...
  const handleExportCSV = () => {
    toast.success("Exporting store_sales_analytics.csv");
  };

  // Filtered Orders Table
  const filteredOrders = useMemo(() => {
    if (orderTableFilter === "all") return RECENT_ORDERS_ADMIN;
    if (orderTableFilter === "unfulfilled")
      return RECENT_ORDERS_ADMIN.filter((o) => o.fulfillmentStatus === "Unfulfilled");
    if (orderTableFilter === "pending")
      return RECENT_ORDERS_ADMIN.filter((o) => o.paymentStatus === "Pending");
    if (orderTableFilter === "fulfilled")
      return RECENT_ORDERS_ADMIN.filter((o) => o.fulfillmentStatus === "Fulfilled");
    return RECENT_ORDERS_ADMIN;
  }, [orderTableFilter]);
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-colors">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-emerald-600">
              <TrendingUp className="size-3.5" />
              <span>+12.8%</span>
              <span className="text-slate-400 font-normal ml-1">98.2% fulfilled</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-colors">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-emerald-600">
              <TrendingUp className="size-3.5" />
              <span>+4.6%</span>
              <span className="text-slate-400 font-normal ml-1">+$7.20 / order</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between hover:border-slate-300 transition-colors">
          ...
          <div className="mt-4">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-xs font-bold text-emerald-600">
              <TrendingUp className="size-3.5" />
              <span>+0.9%</span>
              <span className="text-slate-400 font-normal ml-1">Cart abandon: 24.1%</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-xs space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <Link to="/admin/products" className="text-xs font-bold text-indigo-600 hover:underline">
              View Catalog
            </Link>
          </div>
