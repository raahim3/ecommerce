...
export function AboutPage() {
  ...
  const standards = [
    {
      icon: Compass,
      title: content.standards1Title || "Material Sourcing",
      desc: content.standards1Description || "Certified Grade-A Mongolian cashmere, Tuscan vegetable-tanned leather, and Japanese beta-titanium wireframes.",
    },
    {
      icon: Award,
      title: content.standards2Title || "Small-Batch Runs",
      desc: content.standards2Description || "Manufactured strictly to demand. We produce fewer items with obsessive attention to stitching and tolerances.",
    },
    {
      icon: Leaf,
      title: content.standards3Title || "Eco Packaging",
      desc: content.standards3Description || "Every order arrives in 100% recycled unbleached kraft boxes printed exclusively with biodegradable soy inks.",
    },
    {
      icon: ShieldCheck,
      title: content.standards4Title || "2-Year Warranty",
      desc: content.standards4Description || "We stand behind every item we create with an unconditional two-year repair or replacement guarantee.",
    },
  ];

  const footprintLocations = [
    {
      label: content.footprint1Label || "Biella, Italy",
      title: content.footprint1Title || "Cashmere & Knitwear",
      desc: content.footprint1Description || "Spun in family-run mills operating along the pristine alpine waters of Piedmont since 1948.",
    },
    {
      label: content.footprint2Label || "Kyoto, Japan",
      title: content.footprint2Title || "Ceramics & Diffusers",
      desc: content.footprint2Description || "Hand-thrown organic stoneware ceramics and Hinoki wood oil distillations by master artisans.",
    },
    {
      label: content.footprint3Label || "Porto, Portugal",
      title: content.footprint3Title || "Footwear & Leather",
      desc: content.footprint3Description || "Constructed on natural Margom rubber cup soles with double-stitched vegetable calfskins.",
    },
    {
      label: content.footprint4Label || "Geneva, Switzerland",
      title: content.footprint4Title || "Horology & Crystals",
      desc: content.footprint4Description || "Swiss quartz movement assembly and anti-reflective domed sapphire crystal fabrication.",
    },
  ];
  ...
  return (
    <main className="min-h-screen pb-24 pt-28 lg:pt-36">
      <Head>
        <title head-key="title">{content.metaTitle || `Our Story | ${general.storeName || "Atelier"}`}</title>
        <meta head-key="description" name="description" content={content.metaDescription || content.intro || "Discover Atelier's approach to considered design, enduring materials and responsible craftsmanship."} />
      ...
      <div className="shell">
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-[0.72fr_1.28fr] gap-8 items-center">
          <div className="relative mx-auto w-full max-w-[420px] aspect-[4/5] overflow-hidden rounded-3xl bg-muted shadow-sm">
              <img
              src={content.image || editorialImg}
              alt={content.imageAlt || "Atelier workspace and design sketches"}
              className="h-full w-full object-cover"
            />
          </div>

          <div className="space-y-6 lg:pl-6">
            <span className="eyebrow text-accent">{content.storyEyebrow || "Where it began"}</span>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">
              {content.storyTitle || "A refusal to compromise on materials."}
            </h2>
        ...
        <section className="mt-20 rounded-3xl border border-border/80 bg-surface p-8 sm:p-12 shadow-xs">
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-xs">
            {footprintLocations.map((item, idx) => (
              <div key={idx} className="rounded-2xl bg-muted/40 p-5 border border-border/60">
                <span className="text-accent font-extrabold text-[11px] uppercase">{item.label}</span>
                <h4 className="font-bold text-foreground mt-1 text-sm">{item.title}</h4>
