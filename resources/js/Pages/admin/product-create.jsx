...
export function AdminProductCreatePage({ categories: serverCategories = [], product = null }) {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const selectedCategoryObj = serverCategories.find((c) => c.name === form.category) || serverCategories[0];
      const categoryId = selectedCategoryObj?.id || 1;
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          original_price: form.compareAtPrice ? parseFloat(form.compareAtPrice) : null,
          compare_at_price: form.compareAtPrice ? parseFloat(form.compareAtPrice) : null,
          stock_quantity: parseInt(form.inventoryQty) || 10,
      ...
      toast.success(isEditing ? "Product updated locally." : "Product created locally.");
      navigate("/admin/products");
    } finally {
      setIsSaving(false);
    }
