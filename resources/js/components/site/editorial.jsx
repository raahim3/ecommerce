...
import editorialImageFallback from "@/assets/editorial.jpg";
    ...
    <section className="bg-surface py-14 lg:py-20">
      <div className="shell grid items-center gap-8 lg:grid-cols-2 lg:gap-14">
        ...
        <div className="min-w-0">
          <Reveal>
            <p className="eyebrow">{settings.editorialEyebrow || "Our philosophy"}</p>
            <h2 className="mt-4 text-[clamp(2rem,5vw,3.75rem)] leading-[1] font-extrabold">{settings.editorialTitle || "More than just shopping."}</h2>
          </Reveal>
          <Reveal delay={100}>
            <p className="mt-6 max-w-lg text-lg text-muted-foreground">{settings.editorialDescription || "Thoughtfully selected products. Exceptional quality. Designed for the way you live — and made by people we know by name."}</p>
          </Reveal>
          <Reveal delay={180}>
            <dl className="mt-8 grid max-w-md grid-cols-3 gap-6">
              {stats.map(([value, label]) => <div key={label}><dt className="text-2xl font-extrabold sm:text-3xl">{value}</dt><dd className="mt-1 text-xs tracking-wide text-muted-foreground uppercase">{label}</dd></div>)}
            </dl>
          </Reveal>
          <Reveal delay={240}>
            <Link href={settings.editorialActionUrl || "/about"} className="group mt-8 inline-flex items-center gap-2 text-sm font-semibold">
              <span className="link-underline">{settings.editorialActionLabel || "Our story"}</span>
