...
const BENEFITS = [
  { icon: Truck, title: "Free Shipping", text: (currency) => `On orders over ${currency}` },
        ...
        {BENEFITS.map((benefit, i) => (
          <Reveal key={benefit.title} delay={i * 80} className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">{typeof benefit.text === "function" ? benefit.text(formatPrice(5000)) : benefit.text}</p>
            </div>
          </Reveal>
        ))}
      </div>
